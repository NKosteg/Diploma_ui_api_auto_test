...
def test_create_project(api_client: BoardApi, test_data: dict, delete_project: dict):
    ...
    resp = api_client.create_project(test_data.get("user_project"), test_data.get("user"))
...
def test_update_project(api_client: BoardApi, create_and_delete_project: str, test_data: dict):
...
@allure.suite("Api тест-кейсы")
@allure.feature("READ")
@allure.title("Получить имя по id")
def test_get_name_project(api_client: BoardApi, create_and_delete_project):
    name = api_client.get_project_by_id(create_and_delete_project).get('title')
    print(name)
